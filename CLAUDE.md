...
- `GET /sessions?type=tutoring|class&date=&bid=` / `GET /branches` — Tutoring/Class 녹음을 붙일 LMS 일정 목록. SQL은 **LMS recording 사이트(recording.prestigei.com, Node `server.js`)의 `/api/tutor-schedules`·`/api/class-schedules` 그대로** (튜터링 `t_tutorschedule` scid / 수업 `t_classdate×t_classmain×t_classteacher` cid,cdid,ctid — ctid NULL 가능). `date` 기본 오늘(ET). **지점 접근 범위(오너 2026-09-25):** HQ(`bid` 6) = 전 지점, 그 외 = 본인 `t_user.bid` 하나 — 다른 지점은 목록·지점 목록에 아예 안 나오고 요청하면 403. 멀티 지점 사용자는 DB 에 매핑이 없어 **`RECORDING_MULTI_BRANCH_USERS`** env 예외(`"userId:bid,bid;…"`)로 지정(LMS 가 매핑 테이블을 만들면 교체). `bid` 생략 = 단일 지점은 본인, HQ·멀티는 허용 전체; `0` = 허용 전체. `t_user.tid` 담당 일정은 `mine=true` 우선. (`recording_session_service.allowed_bids / resolve_bid_filter`)
