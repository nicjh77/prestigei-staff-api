...
- `POST /token` — register/refresh the caller's push token (auth: `get_current_user`). Upsert keyed on `(user_id, device_id)`; re-registering reactivates and updates the token. **등록 후 같은 폰의 잔여 행 자동 비활성화 (2026-09-08):** 같은 사용자·플랫폼의 다른 활성 행 중 ① Expo 토큰 ② 같은 `push_token` ③ `device_id` 접두(`{기종}_{OS}_`)가 같은 행(= OS 업데이트로 갈린 같은 폰)을 `is_active=0`. 다른 기종의 두 번째 폰은 유지. 이유: `device_id`에 OS 버전이 들어가 OS 업데이트마다 새 행이 생기고, 옛 행이 활성으로 남아 **같은 폰이 알림을 두 번 받았다**(8월 관측). `dispatch_notification`도 토큰을 중복 제거해 보낸다. 읽음 상태(`t_notification_recipient`)는 사용자 단위라 행 개수와 무관. **앱/기기 상태 동봉 (2026-09-08, 학생 앱과 같은 필드):** `app_version`, `build_number`, `ota`('embedded' | 'YYYYMMDD-HHMM' ET 발행시각), `update_id`, `device_model`, `os_name`, `os_version` — 전부 optional(구버전 앱은 안 보냄, 보낸 것만 갱신) + `last_seen_at = now_et()`. **`push_token` 없이 보내면 "상태만 보고"** — 권한 거부/FCM 토큰 실패 폰도 상태·`last_seen_at`을 남기도록 기존 `(user, device)` 행의 상태 컬럼만 갱신(토큰·`is_active` 유지, 행 없으면 무시). **값은 컬럼 폭에 맞춰 잘라 저장하고 절대 422로 거부하지 않는다** — 참고용 값이 토큰 등록을 막으면 푸시가 끊긴다(긴 `os_name`이 등록을 막았던 학생 앱 사례). 사용자별 앱 상태 조회 SQL은 `changelog.sql` 2026-09-08 항목.
...
- `POST /transcribe` — multipart `audio`(m4a/wav/mp3, ≤300MB·≤2h) + `language`(사이트 드롭다운과 같은 7개, 기본 `en-US`, 패턴 검증) + `mode`(`speech` 단일 화자 / `conversation` 화자 분리, 기본 conversation) + `max_speakers`(2~8, 수업 4) → **202 `{job_id, status:"queued"}`**. 파일은 `tempfile` 로 받아 **`asyncio.create_task` 독립 태스크**에서 Azure 호출 후 **finally 삭제**. **BackgroundTasks 금지 (리뷰 2026-09-25 HIGH):** FastAPI 의 yield 의존성(`get_db`, 인증이 씀)은 BackgroundTasks 까지 끝난 뒤 종료돼 인증 커넥션이 변환 1~3분 내내 풀에 안 돌아온다 → 동시 업로드 몇 건이면 pool 고갈로 전 엔드포인트 500 (/notifications/send 1205 사고와 같은 순서). 요청 안에서 기다리지 않는 이유: Apache 프록시 유휴 타임아웃(기본 60초). **제한**: 사용자당 동시 1건(429), IP 당 시간당 30건(`rate_limit`, Azure 과금 보호), Azure 동시 호출 세마포어 4(10건 몰려도 마지막 대기 ~10분), Content-Length 로 300MB 초과는 본문을 받기 전에 413(Starlette 는 파일 파트를 무제한으로 /tmp 에 다 받은 뒤에야 엔드포인트가 돎). 무음(문장 0개)은 `failed`("No speech detected"). 부팅 시 남은 `rec_*` 임시 파일 sweep(`main.py` lifespan). Apache `LimitRequestBody` 는 실서버에서 70MB 통과 확인(2026-09-25).
...
- `GET /sessions?type=tutoring|class&date=&bid=` / `GET /branches` — Tutoring/Class 녹음을 붙일 LMS 일정 목록. SQL은 **LMS recording 사이트(recording.prestigei.com, Node `server.js`)의 `/api/tutor-schedules`·`/api/class-schedules` 그대로** (튜터링 `t_tutorschedule` scid / 수업 `t_classdate×t_classmain×t_classteacher` cid,cdid,ctid — ctid NULL 가능). `date` 기본 오늘(ET). **지점 접근 범위 (test-report 와 같은 규칙, 오너 2026-09-25):** 접근 가능 지점 = 본인 `t_user.bid` ∪ **`t_permission_income`(memid = t_user.id).branch**(t_branch.shortname → bid). `'HQ'` 권한 행이 있거나 본사(bid 6) 소속이면 `view_all`(전 지점). 다른 지점은 `/branches` 목록에 안 나오고 `/sessions?bid=` 로 요청하면 403. 권한은 LMS 가 관리(앱은 읽기만). `GET /branches` → `{branches, home_bid, view_all, default_all}` — `default_all` 은 HQ 소속만 true(본사엔 수업이 없어 기본 "All"). `bid` 생략 = 본인 소속(HQ 소속은 전체), `0` = 접근 가능 전체. `t_user.tid` 담당 일정은 `mine=true` 우선. (`recording_session_service.branch_access / resolve_bid_filter`)
...
**설정**: `AZURE_SPEECH_KEY`, `AZURE_SPEECH_REGION`(=`eastus2`, LMS 사이트와 같은 구독). 비우면 `/transcribe` 503, 나머지는 정상. **키 값은 로그·응답에 절대 싣지 않는다.** **`UPLOAD_TMP_DIR`** (2026-09-25): 업로드 임시 파일 경로 — **프로덕션 LMS02 의 `/tmp` 는 tmpfs(RAM 3.9G)** 라 비워 두면 60MB×동시 건수(수신본+`rec_*` 두 벌)가 메모리를 먹는다 → `/var/www/staff-app/tmp`(svc-node 쓰기 가능) 로 지정. `main.py` lifespan 이 `tempfile.tempdir` 로 잡아 Starlette 수신본과 `rec_*` 모두 그 디스크를 쓴다. 파티션 여유 ≥3GB 권장(10건 러시 시 순간 ~1.8GB).

- `POST /transcript` — `{client_id, type, transcript, started_at, student_name?, scid? | cid,cdid,ctid?}` (`extra=forbid`) → LMS 테이블 저장 (`transcript_service`, raw SQL). 규칙(오너 확정, 웹과 동일): 튜터 `t_tutor_record(scid)` / 수업 `t_class_record(cid,cdid,IFNULL(ctid,0))` 는 **행 1개, 두 번째 녹음은 기존 transcript 뒤에 `\n\n` 이어붙임**, 앱은 `upddate`(now_et) 만 갱신하고 `submitdate`(제출)는 LMS 몫 — 이미 제출된 행이면 **409**. 상담은 `t_meeting_record(sessionid=yyyymmddhhmi 시작시각, 같은 분 충돌 시 초까지, sessionmemo=학생명 또는 '')` 매번 새 행. **일정 없는 튜터·수업 녹음은 없다**(앱이 일정 필수 — 웹과 동일) → `t_meeting_record` 에는 상담만. 저장 전 일정 실존 검증(scid∈t_tutorschedule / cid+cdid∈t_classdate / ctid∈t_classteacher, 아니면 404). **권한 범위는 웹과 동일하게 제한 없음**(어느 직원이든 어느 지점·교사 일정에나 — 상담사가 지점을 넘나듦; 제품 결정). 중복 방지: `(user_id, client_id)` 를 1시간 기억 → 재전송은 `action:"duplicate"` — **컨트롤러가 명시 `db.commit()` 후 `remember()`** (커밋 실패가 캐시에 남지 않게; 서비스 commit 금지 규칙의 두 번째 예외). LMS 테이블에 유니크 제약 없음(로컬 DDL 확인: `id` auto_increment, `transcript` longtext, `sessionid` varchar(255)). `generated`(예약어)/`sentdate`/`sentto` 는 건드리지 않는다.
